...
class PathMaterial:
    ...
    def __init__(self, obj_path: str, contract: Mapping[str, Any], workdir: str = None):
        ...
        full_hash = contract.get("object", {}).get("sha256_full")
        if full_hash:
            with open(obj_path, "rb") as source:
                if hashlib.sha256(source.read()).hexdigest() != full_hash:
                    raise ValueError("object hash != contract object.sha256_full")
    ...
    # --- full-path marches ------------------------------------------------
    def _validate_path(self, props, increments, times):
        for name, values, shape in (("props", props, (self.nprops,)),
                                    ("increments", increments, (len(increments), self.ntens)),
                                    ("times", times, (len(increments),))):
            array = np.asarray(values)
            if array.dtype.kind not in "fiu" or array.shape != shape:
                raise ValueError(f"{name} must be extracted real numbers of shape {shape}")
            if not np.all(np.isfinite(array)):
                raise ValueError(f"{name} must be finite")
        if not len(increments) or np.any(np.asarray(times) <= 0):
            raise ValueError("path must be nonempty with positive time increments")

    def march_oti(self, props, dstran_path, dt_path):
        """March the OTI eval; return per-increment dict with stress, statev,
        dsigma_dp (nt,nparam), dstatev_dp (nstatev,nparam) at FIXED strain history.

        State derivatives are carried, but displacement-history derivatives are
        not. These partials alone are not an equilibrated solution derivative.
        """
        self._validate_path(props, dstran_path, dt_path)
        nt, ns, nprm = self.ntens, self.nstatev, self.nparam
