...
def _as_tangent_ip(raw, eid) -> np.ndarray:
    ...
    D = np.asarray(raw)
    if D.dtype.kind not in "fiu" or not np.all(np.isfinite(D)):
        raise FieldSensitivityError("tangent requires finite extracted real coefficients; live OTI is not accepted")
...
def _as_dsigma_ip(raw, eid, pname) -> np.ndarray:
    ...
    d = np.asarray(raw)
    if d.dtype.kind not in "fiu" or not np.all(np.isfinite(d)):
        raise FieldSensitivityError("stress derivative requires finite extracted real coefficients; live OTI is not accepted")
...
def solve_field_sensitivities(
    ...
    for eid, el in elements:
        ...
        if bbar:
            from ..replay.kinematics import element_tangent_bbar
            k_e = element_tangent_bbar(Xe, D_ip)
        else:
        ...
        for col, pname in enumerate(params):
            ...
            if bbar:
                from ..replay.kinematics import element_internal_force_bbar
                r_e = element_internal_force_bbar(Xe, dsig)
            else:
...
def fields_from_statev(
    ...
    for eid, per_ip in statev.items():
        try:
            ...
            eid_i = eid
        arr = np.asarray(per_ip)
        if arr.dtype.kind not in "fiu" or not np.all(np.isfinite(arr)):
            raise FieldSensitivityError("SDVs require finite extracted real coefficients; live OTI is not accepted")
