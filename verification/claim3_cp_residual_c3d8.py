...
What this script does (current packages)
    ...
    * One C3D8 (unit cube), bottom face y=0 fixed, top face y=1: u1 = gamma*H,
      ...
      the residual method (verification/c3d8_residual.py): K du/dp = -dR/dp with
    ...
    * References: (a) hand-derived chain rule of the same update
      (verification/cpflow_analytic.py), NRMSE = RMSE / max|reference| per
    ...
    * Cost: (a) the Python analysis driver, plain vs enriched vs 13 re-solves;
      (b) the compiled kernel (verification/cp_timing_driver.f90 linked with the
    ...
    * ``--abaqus``: the same single C3D8 in Abaqus with the unchanged umat.for
    ...
    python verification/claim3_cp_residual_c3d8.py [--abaqus]
"""
...
from verification import c3d8_residual as C  # noqa: E402
from verification import cpflow_analytic as A  # noqa: E402
...
def main(argv=None) -> int:
    ...
    record["analytic_chain_rule"] = {
        "reference": "verification/cpflow_analytic.py (hand-derived implicit-function chain rule)",
    ...
    # ---- Abaqus -----------------------------------------------------------------
    if args.abaqus:
        from verification import claim3_abaqus
