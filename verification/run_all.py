...
"""Run every presentation-claim reproduction and write one summary.
    ...
    python verification/run_all.py                 # offline parts of all claims
    python verification/run_all.py --abaqus        # plus the Abaqus jobs (claims 3, 4)
    python verification/run_all.py --quick         # reduced cases, a few minutes

Environment (see docs/VERIFICATION_RECORD.md): PYTHONPATH must put this
...
``--out`` (default verification/results/, git-ignored).

The summary ``presentation_summary.json`` lists, per claim, the slide's value
and the measured one. ``verification/expected/presentation_summary.json`` is the
committed copy from the run documented in docs/VERIFICATION_RECORD.md.
"""
...
from verification.common import default_out, default_work, provenance, require_worktree_imports, write_json  # noqa: E402
