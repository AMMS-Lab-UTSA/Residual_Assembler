...
Jobs run one at a time, named ``verify_c3_*``. A job counts as completed only
...
from verification import c3d8_residual as C
from verification.common import umat_repo

JOB_PREFIX = os.environ.get("PRESENTATION_ABAQUS_PREFIX", "verify_c3")
...
def deck(props: Sequence[float], n_increments: int, dt: float, gamma: float, mesh: int = 1) -> str:
    ...
    lines = ["*HEADING", f"claim 3: m5_cpflow simple shear, {mesh}x{mesh}x{mesh} C3D8",
             "*NODE, NSET=ALLN"]
