...
> For the full picture — inputs, outputs, the methodology, how it is tested, and how
> it connects to the OTIS UMAT transformer — read [METHODOLOGY.md](METHODOLOGY.md).

**Install:** `pip install -e .` (exposes `resasm`). Assembly needs nothing else.

## Verify against an Abaqus run — one file, one command

If you have a solved Abaqus job and want to confirm the assembler reproduces it,
you write **one** JSON file and run **one** command. You do not manage a separate
mesh file and field file — the tool exports the fields from the ODB for you.
...
It exports the ODB fields (`abaqus python`, done for you), assembles the residual,
and checks that the free-DOF residual is ~0 and the assembled reaction equals the
Abaqus `RF`. A worked, runnable example (a single elastic C3D8 that passes to
~1e-16) is in [`examples/abaqus_elastic_c3d8/`](examples/abaqus_elastic_c3d8/).
No Abaqus on this box? Set `"fields": "fields.json"` instead of `"odb"` to run
against an export made elsewhere. Schema:
[`schemas/resasm_verification_job_v1.schema.json`](schemas/resasm_verification_job_v1.schema.json).
