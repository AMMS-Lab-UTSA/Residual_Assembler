...
# This verification parses a third-party, permissively-licensed Oxford
# crystal-plasticity example mesh (ngrilli HCPnoTwin/Compression111.inp). That
# mesh is an optional *data fixture*, not a code dependency, and is not vendored
# in every checkout. When it is absent the test skips with this exact reason
# rather than erroring on a FileNotFoundError that happens to contain "abaqus".
INP_MISSING_REASON = (
    "fixture mesh not vendored: sources/permissive/ngrilli_Oxford_Crystal_Plasticity/"
    "ExampleInputFiles/HCPnoTwin/Compression111.inp (third-party permissively-licensed "
    "example; set it up to run the formulation-agnostic assembler cross-check)"
)
INP_AVAILABLE = os.path.exists(INP)
...
def am():
    ...
    if not INP_AVAILABLE:
        pytest.skip(INP_MISSING_REASON)
...
def main():
    ...
    if not INP_AVAILABLE:
        # exit non-zero with a recognizable message so the script runner records
        # a genuine SKIP (not a PASS, which would misrepresent coverage, nor a
        # FAIL, since the mesh is an optional external fixture).
        print("  SKIP: %s" % INP_MISSING_REASON)
        return 2
