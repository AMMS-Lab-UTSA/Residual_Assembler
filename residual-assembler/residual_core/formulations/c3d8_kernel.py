...
# ---------------------------------------------------------------------------
# Mean-dilatation (B-bar) small-strain assembly -- matches Abaqus C3D8
# ---------------------------------------------------------------------------
# Abaqus's fully-integrated first-order solids (C3D8) use SELECTIVE REDUCED
# integration: the deviatoric response is integrated at the 8 Gauss points, but
# the VOLUMETRIC (dilatational) response uses the element-average dilatation
# ("mean-dilatation" / B-bar), to avoid volumetric locking. A pure full-
# integration kernel does NOT reproduce Abaqus C3D8 stresses/tangent -- they
# differ by a per-IP hydrostatic term. Verified against a real Abaqus 2021 ODB:
# mean-dilatation stress recovery matches Abaqus S to ~1e-7 (single-precision
# ODB floor), full integration is off by O(1). These helpers give the engine an
# Abaqus-consistent assembly for exported C3D8 fields; the full-integration path
# above is unchanged.

def bbar_b_matrices(Xe, gauss=ABAQUS_C3D8_GAUSS):
    """Mean-dilatation (B-bar) B matrices at the 8 IPs plus their detJ.

    The dilatational part of each B is replaced by its volume average:

        vol_row[q] = B[q][0] + B[q][1] + B[q][2]           (d theta / d u at q)
        vol_bar    = (1/V) sum_q vol_row[q] detJ_q w_q     (mean dilatation grad)
        Bbar[q]    = B[q];  rows 0,1,2 += (vol_bar - vol_row[q]) / 3

    Returns ``(Bbar_list (8 x (6,24)), detJ_list (8,))``.
    """
    Xe = np.asarray(Xe, dtype=float)
    points, weights = gauss.points, gauss.weights
    n = len(weights)
    Bs, dets, volrows = [], [], []
    for q in range(n):
        B, detJ, _ = _b_from_coords(Xe, points[q])
        Bs.append(B)
        dets.append(detJ)
        volrows.append(B[0] + B[1] + B[2])
    V = sum(dets[q] * weights[q] for q in range(n))
    vol_bar = sum(volrows[q] * dets[q] * weights[q] for q in range(n)) / V
    Bbar = []
    for q in range(n):
        B = Bs[q].copy()
        corr = (vol_bar - volrows[q]) / 3.0
        B[0] = B[0] + corr
        B[1] = B[1] + corr
        B[2] = B[2] + corr
        Bbar.append(B)
...
def element_tangent_bbar(Xe, Dmat_ip, gauss=ABAQUS_C3D8_GAUSS):
    """Small-strain element tangent with mean-dilatation B-bar (Abaqus C3D8):

        K = sum_q  Bbar_q^T D_q Bbar_q  detJ_q w_q .

    Xe      : (8,3) reference coords.
    Dmat_ip : (8,6,6) per-IP material tangent (or (6,6) broadcast).
    """
    Dmat_ip = np.asarray(Dmat_ip, dtype=float)
    if Dmat_ip.shape == (6, 6):
        Dmat_ip = np.broadcast_to(Dmat_ip, (8, 6, 6))
    Bbar, dets = bbar_b_matrices(Xe, gauss)
    weights = gauss.weights
    K = np.zeros((24, 24), dtype=float)
    for q in range(len(weights)):
        K += (Bbar[q].T @ Dmat_ip[q] @ Bbar[q]) * dets[q] * weights[q]
    return K


def element_internal_force_bbar(Xe, field_ip, gauss=ABAQUS_C3D8_GAUSS):
    """B-bar internal-force / residual-derivative assembly (Abaqus C3D8):

        f = sum_q  Bbar_q^T field_ip[q]  detJ_q w_q .

    Use with the per-IP stress (internal force) or the per-IP dsigma/da (the
    residual parameter-derivative column). field_ip : (8,6) Voigt.
    """
    field_ip = np.asarray(field_ip, dtype=float)
    Bbar, dets = bbar_b_matrices(Xe, gauss)
    weights = gauss.weights
    f = np.zeros(24, dtype=float)
    for q in range(len(weights)):
        f += (Bbar[q].T @ field_ip[q]) * dets[q] * weights[q]
    return f
