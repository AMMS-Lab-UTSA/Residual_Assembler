...
def _cmd_verify_job(args):
    """Single-file path: assemble + verify the residual against an Abaqus run,
    driven by one job file (model + odb/fields + mode)."""
    from resasm_user.abaqus_job import run_job, render, JobError
    try:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s = sub.add_parser("verify-job",
                       help="ONE-FILE path: assemble + verify against an Abaqus "
                            "run from a single job file (model + odb/fields + mode)")
    s.add_argument("job", help="job file (.json/.yml): model, odb or fields, mode")
    s.set_defaults(func=_cmd_verify_job)

    s = sub.add_parser("project",
                       help="project-oriented workflow: create/inspect/configure/"
                            "prepare/run/validate/report/status")
    s.add_argument("args", nargs=argparse.REMAINDER,
                   help="a project subcommand and its arguments")
    s.set_defaults(func=_cmd_project)
    return p


def _cmd_project(args):
    from resasm_user.project.cli import main as project_main
    return project_main(args.args)
