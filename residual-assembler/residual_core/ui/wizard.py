...
class ResidualProblem:
    def __init__(self, model: Model, formulation_registry=None,
        ...
        self.results = None                     # DerivativeFields, once attached
    ...
    def attach_results(self, source: Any) -> "ResidualProblem":
        """Attach exported solver fields.

        Three shapes are accepted:

        * a ``resasm_derivative_fields_v1`` export (dict or ``.json`` path): the
          full per-IP derivative fields for the field-driven residual method.
          It is validated + normalized and exposed as ``self.results`` (a
          :class:`~residual_core.io.derivative_fields.DerivativeFields` with
          ``.displacements``, ``.statev``, ``.metadata``). Any embedded
          ``stress_ip`` is also kept for stress-driven verification.
        * a dict already shaped as ``{'stress_ip': {eid:(nip,6)}}`` (or just
          ``{eid:(nip,6)}``) -- stress-driven mode.
        * a path to a plain JSON field export. Binary ODB reading is delegated to
          ``io/abaqus_odb_export`` and requires Abaqus.
        """
        from ..io.derivative_fields import (is_derivative_field_dict,
                                            load_derivative_fields)

        if isinstance(source, dict):
            if is_derivative_field_dict(source):
                self.results = load_derivative_fields(source)
                self._keep_stress_from(source)
            else:
                self._fields = (source if "stress_ip" in source
                                else {"stress_ip": source})
        elif isinstance(source, str) and source.lower().endswith(".json"):
            with open(source, "r", encoding="utf-8") as fh:
                d = json.load(fh)
            if is_derivative_field_dict(d):
                self.results = load_derivative_fields(d)
                self._keep_stress_from(d)
            else:
    ...
    def _keep_stress_from(self, d: Dict[str, Any]) -> None:
        """Retain any embedded integration-point stress alongside a derivative
        export, so stress-driven verification still works on the same file."""
        stress = d.get("stress_ip")
