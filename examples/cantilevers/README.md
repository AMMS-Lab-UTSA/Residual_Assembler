# Two full-size cantilevers: J2 plasticity and FCC crystal plasticity

Two nonlinear C3D8 models for the full sensitivity workflow:

| model | mesh | DOF | integration points | steps | parameters |
| --- | --- | ---: | ---: | ---: | --- |
| `j2` | 48 x 16 x 2 C3D8 | 7,497 | 12,288 | 40 | E, nu, SIGY0, H = 200000, 0.3, 250, 2000 |
| `fcc` | 24 x 8 x 2 C3D8 | 2,025 | 3,072 | 25 | C11, C12, C44, g0, gsat, h0, a, q, gd0, m = 168000, 121000, 75000, 13, 55, 800, 2, 1.4, 0.001, 0.05 |
...
| file | what it does |
| --- | --- |
| `gen_cantilever.py` | writes a deck: `python gen_cantilever.py j2 --out j2/cantilever_j2_nominal.inp` (`--props` overrides PROPS) |
...
```sh
mkdir -p j2 && python $EX/gen_cantilever.py j2 --out j2/cantilever_j2_nominal.inp
cd j2 && abaqus job=cantilever_j2_nominal input=cantilever_j2_nominal.inp \
    user=$UMAT/parameter_sensitivity/models/m3_j2/umat.for double=both interactive && cd ..
grep "COMPLETED SUCCESSFULLY" j2/cantilever_j2_nominal.sta     # the verdict; see below

umat-oti-provider build $UMAT/parameter_sensitivity/models/m3_j2/contract_v2.json --out provider_j2
resasm request --model j2/cantilever_j2_nominal.inp --odb j2/cantilever_j2_nominal.odb \
    --material provider_j2/umat_m3_j2_oti.obj --request $EX/j2_request.json --out j2_results
```

`resasm request` sees a model outside its bounded single-material scope and hands
...
abaqus python $EX/export_odb.py -- j2/cantilever_j2_nominal.odb j2/cantilever_j2_nominal_fields.npz
python $EX/fd_reference.py j2 .
```

The full comparison (both cantilevers, recorded and re-equilibrated, Abaqus
FD, figures and the weighted per-increment shares) is
`scripts/replay_history_cantilevers.py`; its results and the measured numbers
are in [docs/evidence/history_replay_cantilevers.md](../../docs/evidence/history_replay_cantilevers.md).
