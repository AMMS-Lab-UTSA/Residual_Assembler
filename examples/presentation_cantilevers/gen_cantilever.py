#!/usr/bin/env python3
"""Write the IMQCAM Annual Meeting cantilever decks as Abaqus input files.

The original decks and ODBs of slides 15 and 39 were not preserved, so the
models are rebuilt from the numbers the slides print:
...
the ODB. PROPS can be overridden for the finite-difference reruns.
    ...
    python gen_cantilever.py j2  --out j2/claude_j2_nominal.inp
...
MODELS = {
    ...
    "fcc": {"n": (24, 8, 2), "push": 0.25, "steps": 25, "depvar": 12,
            # slide 15: C11-C12-C44 = 168-121-75 GPa, g0 = 13 MPa, gsat = 55,
            # h0 = 800, a = 2, q = 1.4, gamma0 = 0.001 /s, m = 0.05 (MPa, s).
            # (The model's contract_v2.json validation values differ: g0 16,
            # gsat 40, h0 300 -- that case is kept in fcc_contract_values/.)
...
def deck(model, props):
    ...
    nx, ny, nz = spec["n"]
    lines = ["*Heading",
             f"IMQCAM {model} cantilever: {nx}x{ny}x{nz} C3D8, "
             f"tip -{spec['push']} mm in {spec['steps']} steps (reconstructed)",
             "*Node"]
    for k in range(nz + 1):
        for j in range(ny + 1):
            for i in range(nx + 1):
