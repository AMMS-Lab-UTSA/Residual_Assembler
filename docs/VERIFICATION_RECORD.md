# Verification record: claims, commands, references, measured values

Claims are numbered by the slide of the project presentation (11 August 2026,
42 slides) that states them.
...
$PY verification/run_all.py            # offline parts of every claim (about 15 min)
$PY verification/run_all.py --abaqus   # plus the Abaqus jobs of claims 3 and 4 (one job at a time)
$PY verification/run_all.py --quick    # reduced cases (a few minutes)
$PY -m pytest -q tests/verification -m "not abaqus and not slow"
```

Each claim also runs on its own (`verification/claimN_*.py --help`). Results are
written to `verification/results/` (git-ignored; small JSON/CSV) and summarised in
`verification/results/presentation_summary.json`; the committed copy of the run
reported here is `verification/expected/presentation_summary.json`. Requirements:
...
**Protocol.** One driver (`verification/claim2_flowrule_driver.f90`) compiled with
...
UMAT; the residual method (`verification/c3d8_residual.py`: K du/dp = −dR/dp with
...
`verify_c4_<case>_{orig,oti}`, `double=both`; a job counts only with the
...
(`imq_abaqus/verification/claim4_before_transformer_fix.json`).

**Re-run after UMAT `5b97c2f` (2026-09-18).** That commit keeps INTEGER
variables out of the promotion and lets the rewrite read the implicit types of
the lifted helper routines' dummies. It changed the transformed sources of seven
benchmark contracts: UMAT_HIN, PCL, PCLI, PCLI_R, PCLK, PCO and VPDCL_R
(re-transforming the other twelve gives byte-identical files, so their rows
stand). The seven, and the VPDCL_R plane-strain variant, were run again in the
paired Abaqus 2021.HF5 validation (UMAT `5b97c2f`, this repository `1bc2009`);
the rows marked *re-run* below are those measurements. Every job of the six
slide cases completed (completion mark in the `.sta`), and HIN, PCL, PCLI,
PCLI_R, PCLK and PCO again reproduce the original DDSDDE exactly (0 / 0 over 4
compared increments; HIN's deck has 1), with identical stress and state
variables. VPDCL_R's 3-D contract fails again in both builds (the original's
local plasticity iteration does not converge in increment 2 and the job aborts at
the UMAT's own diagnostic WRITE; the transformed build stops with a segmentation
fault in increment 1); its plane-strain variant gives 0.3673 / 3.2e-04 again.
Before `5b97c2f`, UMAT_HIN's transformed build passed wrongly typed arguments to
its helper routines (the OTI copies of the INTEGER flags ISTEP, ICYCLE and IFLAG
to INTEGER dummies, plain REAL values such as RSTE and CTIME to OTI dummies); its
single-increment deck did not reveal that, so the earlier HIN row is superseded
by the re-run, not confirmed by it. The claim-4 counts are unchanged.
...
| UMAT_HIN | 0 / 0 pass (*re-run*; supersedes the pre-`5b97c2f` row) | – | 0 / 0, Exact |
| UMAT_NKH_1.02 | 2657 / 8.9e-01 FAIL (uninitialised DTHTA) | **0.7234** / 7.5e-04 pass | 0.72 / 2.1e-3, notching |
| UMAT_PCL | 0 / 0 pass (*re-run*) | – | 0 / 0, Exact |
| UMAT_PCLI | 0 / 0 pass (*re-run*) | – | 0 / 0, Exact |
| UMAT_PCLI_R | 0 / 0 pass (*re-run*) | – | 0 / 0, Exact |
| UMAT_PCLK | 0 / 0 pass (*re-run*) | – | 0 / 0, Exact |
| UMAT_PCO | 0 / 0 pass (resolved helper closure; *re-run*) | – | 0 / 0, Exact |
...
| UMAT_VPDCL_R (not on the slide) | both jobs fail (3-D deck; source in bounds only for NTENS = 4; *re-run*) | 0.3673 / 3.2e-04 pass (plane-strain deck; *re-run*) | – |
...
| 9–12 | developer/collaborator split (REAL_UMAT.obj, OTI_UMAT.obj, Mapping.json, transform_report.txt; Analysis.inp/.odb + sensitivity_request.json → sensitivity_results.json, sensitivity_tables.csv, run_report.txt) | **Works.** Developer: `umat-oti-provider build CONTRACT --out DIR --regular-object REAL_UMAT.obj` (or the UMAT GUI's Parameter Sensitivities tab) writes the object with the ORIGINAL and the OTI lift, `REAL_UMAT.obj` (the same ORIGINAL compile; `--abaqus-toolchain` builds it with `abaqus make`), the completed contract and the report, and verifies every DSIGMA_DP / DSTATEV_DP / DDSDDE entry against centred FD of the separately compiled ORIGINAL (m3_j2: 628 agree, 240 consistent with zero, 0 disagree; m6_fcc with slide-15 values on a tension-with-shear path: 4,556 / 1,572 / 112 unresolved / 0). Objects carry no machine paths and rebuild byte-identically. Collaborator: `resasm request --model Analysis.inp --odb Analysis.odb --material OTI_UMAT.obj --request sensitivity_request.json --out results` (or the RA GUI's first tab) writes the three files; the bounded example and both full-size cantilevers run through it. No UMAT source is read: the clean-install gate reruns the bounded example with every open of a Fortran file denied (except the link shim it generates itself) and gets byte-identical outputs. |
| 13 | sensitivity verification table | claim 1 |
| 14 | section title | – |
| 15, 33, 39 | FCC and J2 cantilevers (384 / 1 536 C3D8, 25 / 40 steps, weighted sensitivities E 96 %, σ_y0 71 %) | **Works at full size** from Abaqus 2021.HF5 ODBs of the decks in `examples/cantilevers/` (rebuilt from the slide numbers; the originals were not preserved): `resasm request` routes both to the history engine; J2 10 s recorded / 25 s re-equilibrated, FCC 19 s / 72 s. OTI vs whole-model FD of the ORIGINAL UMAT at full size: FCC all 10 parameters ≤ 6.7e-7; J2 E 6.6e-8, nu 5.8e-8 (U 8.4e-6 at its FD spread), SIGY0 7.9e-9, H 5.4e-6, except at increments where points sit on the yield surface and the FD has no plateau. Homogeneity identity (Σ p ∂Q/∂p = Q or 0, every increment, re-equilibrated): J2 1.0e-12, FCC 1.2e-13. Slide 33 **not reproduced as stated**: elastic E share 98.2 % (slide ~96 %); the σ_y0 71 % / E 24 % pair occurs at step 16, not at yield onset (step 7). Evidence: `docs/evidence/history_replay_cantilevers.md`. |
| 16–18, 40–42 | GUI walk-through (transform, provider build, residual solve) | **Works.** UMAT GUI: Constitutive Jacobian tab (upload a UMAT, NTENS, Transform; byte-identical to `umat-oti jacobian`) and Parameter Sensitivities tab (parameter table, DSIGMA_DP / DSTATEV_DP ticks, Build); RA GUI: Sensitivity Request / Solve tab (parameters from Mapping.json, outputs and region, full-field table). Browser tests: UMAT `pytest -m gui` 5 passed, RA 1 passed on a real ODB. Screens and evidence: UMAT `docs/GUI.md`, `docs/evidence/gui_screens.md`. |
| 19 | 207 UMATs collected by web scraping, full set re-run after every fix | **Re-run at the final transform** (pass16, fingerprint `dbe9f928191e1d43`, 2026-09-19): 391 acquired sources re-transformed (240 transform), all 240 run in Abaqus 2021.HF5; 43 of the 260 adequately specified genuine UMATs clear every acceptance gate, the same 43 as the three passes before it. The slide's 207 is an older count of the acquisition. Evidence: [final_refreeze.md](evidence/final_refreeze.md); census in UMAT_source_transformation `paper_results/corpus/`. |
...
See `docs/evidence/verification_record_run.md`. In the UMAT repository six
transformer defects found by these reproductions and by the corpus rerun were
fixed, each with a regression test that fails without the fix:

| Commit | File | Defect |
|---|---|---|
| `7c4975c` | `src/umat_oti/transform/source_transform.py` | labelled statements lost their label (PCL, PCLI, PCLI_R, PCLK did not compile) |
| `7c4975c` | `src/umat_oti/fortran/regions.py` | inputs of a DDSDDE used as the predictor stiffness were skipped (VPDCL, NKH wrong stress) |
| `b124e39` | `src/umat_oti/transform/source_transform.py` | a DATA constant listed under promote refused the file (HIN) |
| `24143f8` | `src/umat_oti/services/transformation.py`, `benchmarks/UMAT_PCO.json`, `tools/run_completed_json_batch.py` | a contract could not name the published helper sources its UMAT calls (PCO) |
| `f11806f` | `src/umat_oti/transform/source_transform.py` | a continued assignment to a real variable from a promoted value was rewritten from its first line only (one corpus source stopped compiling) |
| `5b97c2f` | `src/umat_oti/transform/source_transform.py` | integer flags on the stress path were promoted and real actuals reached hypercomplex dummies of lifted helpers (HIN: argument type mismatches that gfortran 10+ refuses; stress 63 % off on an offline path, now within 1.1e-9) |
