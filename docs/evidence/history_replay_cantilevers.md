# Evidence: general history replay for the presentation cantilevers (The history-replay work stream)

Branches `dev/history-replay` in both repositories (RA worktree `ra-history-replay`,
UMAT worktree `umat-history-replay`), from the 11:05 snapshots (RA `d8ea804`, UMAT
...
reruns) live outside the repositories in `imq_abaqus/history_replay/`; the small
tables and figures are in `docs/evidence/history_replay_cantilevers/`.
...
2021.HF5 data (`imq_abaqus/cantilevers/`), verified against Abaqus
...
document): new modules only, the recovery line's `connected.py`, `presentation*.py`,
...
Environment as in the brief (`PYTHONPATH=ra-history-replay:umat-history-replay/src:otilib`,
`UMAT_OTI_REPO=umat-history-replay`).

```sh
W=imq_abaqus/history_replay; D=imq_abaqus/cantilevers
...
resasm history --model $D/j2/cantilever_j2_nominal.inp --odb $D/j2/cantilever_j2_nominal.odb \
...
python scripts/replay_history_cantilevers.py --abaqus-data $D --providers $W/providers \
    --work $W/cantilevers --evidence docs/evidence/history_replay_cantilevers --tight-fd j2=$W/j2_fd_tight
# tight-tolerance Abaqus reruns of the J2 cantilever (25 jobs, one at a time, hr_ prefix):
python scripts/replay_history_abaqus_fd.py --deck $D/j2/cantilever_j2_nominal.inp \
    ...
    --prefix j2cant --work $W/j2_fd_tight
# whole-model ORIGINAL-UMAT FD at full size, one process per parameter:
python scripts/replay_history_fd_fullsize.py --deck $D/j2/cantilever_j2_nominal.inp \
    --fields $D/j2/cantilever_j2_nominal_fields.npz --object $W/providers/prov_j2/umat_m3_j2_oti.obj \
    --parameters E --steps 1e-3,1e-4,1e-5 --rtol 1e-12 --out $W/fd_fullsize/j2_E.json
```

My ODB exporter reproduces the lead's export of `cantilever_j2_nominal.odb` bit
for bit (all 9 arrays identical) in 3.2 s.

The slide-12 command itself from the ODB (`resasm history --model
cantilever_j2_nominal.inp --odb cantilever_j2_nominal.odb --material umat_m3_j2_oti.obj
...
One cProfile of the J2 recorded replay (`history_replay_cantilevers/j2_profile.txt`): 11.2 s,
...
field scale; per-increment data in `history_replay_cantilevers/fullsize_python_fd_summary.json`). This is the
...
`history_replay_cantilevers/{j2,fcc}_abaqus_fd*_reequilibrated.csv` (recorded-mode summaries in
`history_replay_cantilevers/cantilever_summary.json`).
...
sigma_vM. Tables: `history_replay_cantilevers/{j2,fcc}_shares_{reequilibrated,recorded}.csv`;
figures `history_replay_cantilevers/j2_weighted_shares.png`, `j2_steps_SIGY0.png`,
...
- RA: `tests/replay_history` 47 offline tests passed (provider 6, engine 16,
  ...
  (`RESASM_ABAQUS_JOB_PREFIX=hr_`, 21.6 s). Full offline RA suite
...
- Large data (not committed): `imq_abaqus/history_replay/cantilevers/` (full fields,
