# Evidence: the verification-record run (2026-09-18)

Branch `dev/verification` in both repositories (worktrees `ra-verification`,
`umat-verification`), from the snapshot commits RA `d8ea804`, UMAT `f6fcc42`. Mission:
make the quantitative claims of the project presentation (11 August
...
is `docs/VERIFICATION_RECORD.md`; this file records how the work was done, what
...
`/home/ammslab3/softwarex_work/imq_abaqus/verification/` (never inside a worktree).
...
Abaqus: one job at a time, every job named `verify_*`, judged by
...
* Slides 28–32: the original 12-slip "Constitutive_UMAT.f" study (figures dated by
  2026-06-19 in an earlier project presentation) is not on this
...
| File | Purpose |
|---|---|
| `verification/__init__.py`, `common.py` | provenance, provider build (`python -m umat_oti.provider build`), FD ladder and plateau |
| `verification/claim1_sensitivity_sweep.py` | slide 13 |
| `verification/claim2_flowrule_jacobian.py`, `claim2_flowrule_driver.f90` | slides 26–27 |
| `verification/claim3_cp_residual_c3d8.py`, `c3d8_residual.py`, `cpflow_analytic.py`, `cp_timing_driver.f90`, `claim3_abaqus.py` | slides 28–32 (generic small-strain C3D8 residual-method driver with nonzero prescribed displacements; hand-derived chain rule of m5_cpflow; compiled timing driver; Abaqus deck + ASCII .fil reader) |
| `verification/claim4_benchmark_ddsdde.py` | slide 8 (paired Abaqus, verify_ job names, documented variants) |
| `verification/claim5_constitutive_jacobians.py` | slide 25 (all six internal-Jacobian symbols; uninitialised-read gate) |
| `verification/run_all.py` | runs all claims, writes `presentation_summary.json` |
| `verification/expected/presentation_summary.json` | committed summary of the recorded run |
| `tests/verification/*` | one end-to-end test module per claim (conftest + 5 modules) |
| `docs/VERIFICATION_RECORD.md`, `docs/evidence/verification_record_run.md` | documentation |

UMAT (`umat-verification`): `docs/VERIFICATION_RECORD.md`,
...
|---|---|---|---|
| RA | `.gitignore` | `verification/results/` ignored | run-time results |
...
fails (it passes on the snapshot without these edits). the recovery line edits the same two
...
|---|---|---|
| RA `tests/verification`, offline incl. `slow` | `pytest -q tests/verification -m "not abaqus"` | 7 passed (4 min 53 s) |
| RA `tests/verification`, Abaqus | `pytest -q tests/verification -m abaqus` | 2 passed (claim-3 nominal job, claim-4 `elastic` pair) |
...
`verification/expected/presentation_summary.json` (and the per-claim JSON in
`verification/results/`). Headline numbers: claim 1, 20/20 models, 84 directions,
