...
This records how the evidence was made current again, following the procedure in `schemas/transform_generation.json`
(`how_to_update`).
...
| step | command (from the UMAT_source_transformation root, except step 6) | result |
| --- | --- | --- |
...
| 3. rebuild the registry | `python tools/build_corpus_registry.py --transform <run>/transform_all_pass16.json --abaqus <run>/pass16/results/store_verification.jsonl --store-fingerprint dbe9f928191e1d43 --audit-refusals` | UMAT_source_transformation `paper_results/corpus/` rewritten, with the offline `ifort -syntax-only` audit of every refused source redone |
...
| 5. generation and lock | fingerprint written to UMAT_source_transformation `src/umat_oti/contract/schemas/transform_generation.json`, `umat_oti.contract.schema.write_lock()`, both copied byte for byte to `schemas/` here | combined lock digest `e59a274028f0e579c5e1f25b9362f920ac24cebd5229bd30794b75dc790ae749` |
| 6. Residual Assembler fixtures | `scripts/regenerate_recovery_fixture.py` (this repository) for `isotropic-elasticity--f7eb90376a` and `j2_props--2feae9f158` (loading from the retained pass12 manifests, deck byte-identical) | four Abaqus jobs; both verified (tangent agreement 1.1e-14 and 8.3e-11 over a step-size plateau). Every number is identical to pass15 and pass14; only the generation time and the fingerprint (`dbe9f928191e1d43`) differ. `residual_core.core.fixture_residual_check`: elasticity 1 held / 2 not established, J2 32 held / 3 not established, the same pattern as before |
...
flags of the UMAT_source_transformation benchmark `UMAT_HIN` had been promoted because they sit on the
...
0.63 (UMAT_source_transformation
[tests/test_lifted_helper_arguments_match_their_dummies.py](https://github.com/AMMS-Lab-UTSA/UMAT_source_transformation/blob/main/tests/test_lifted_helper_arguments_match_their_dummies.py)). No other
...
UMAT_source_transformation's verified collection under `umat/` (44 material contracts, pass12,
`94a92c01814f107a`) and the ten fixtures here under
`tests/fixtures/historical/94a92c01814f107a/` are not regenerated here.
