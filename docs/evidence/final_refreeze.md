...
| 3. rebuild the registry | `python tools/build_corpus_registry.py --transform <run>/transform_all_pass15.json --abaqus <run>/pass15/results/store_verification.jsonl --store-fingerprint 16c9f305df378089 --audit-refusals` | UMAT_source_transformation `paper_results/corpus/` rewritten. The refused set changed, so the offline `ifort -syntax-only` audit of every refused source was redone |
...
| 5. generation and lock | fingerprint written to UMAT_source_transformation `src/umat_oti/contract/schemas/transform_generation.json`, `umat_oti.contract.schema.write_lock()`, both copied byte for byte to `schemas/` here | combined lock digest `036e7797d203d3adf2b1f492d41dc65a1d320197016e6d95cbabfa3b8415d789` |
