# The GUI work stream: the slide-18/42 Solve screen

Branch `dev/gui`, from the snapshot `d8ea804` of the recovery line's
`imq-ra-recovery` working tree. This file lists **every edit to a file the recovery line
also edits**, for the merge. New files follow.

## Edits to files the recovery line owns or edits
...
the recovery line's AppTest (`tests/integration/test_presentation_request.py::test_gui_shared_service_with_compiled_core`)
...
with output under `../imq_abaqus/gui/`: `passed: true`, three downloads,
no horizontal overflow at 390 px, and the server stopped.

### `docs/REQUEST_INTERFACE.md`
...
### `tests/framework/test_gui_is_a_thin_cli_front_end.py` (pre-existing, not the recovery line's)
...
- `tests/gui/test_solve_screen.py` (AppTest, offline suite, 6 tests),
  `tests/gui/test_solve_screen_browser.py` (`-m gui`, abaqus, 1 test),
...
Environment: `PYTHONPATH=<this worktree>:<umat-gui>/src:<otilib build_py311>`,
`UMAT_OTI_REPO=<umat-gui>`, Python 3.11 venv, gfortran 9.4, Abaqus
2021.HF5.

```text
python -m pytest -q -p no:cacheprovider tests/gui/test_solve_screen.py \
...
1e-09"), as docs/REQUEST_INTERFACE.md says it may be. Independent
