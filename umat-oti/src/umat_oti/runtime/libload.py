"""Cross-platform loading of the gfortran-built validation libraries.

Program 1 compiles two throw-away shared libraries during validation -- one
around the original UMAT, one around the transformed OTI object -- and calls
them through :mod:`ctypes`.  Windows and Linux differ in two ways that matter:

* **File name.**  A shared library is ``.so`` on Linux and ``.dll`` on Windows.
  Handing a Windows loader a file called ``oti.so`` happens to work, but naming
  it correctly keeps the build directory readable and matches what every other
  Windows tool expects.

* **Dependency resolution.**  A gfortran-built library imports ``libgfortran``
  (and, depending on the model, ``libquadmath`` / ``libwinpthread`` /
  ``libgcc_s``).  On Linux ``ld.so`` finds those through the normal search path.
  On Windows, Python 3.8+ deliberately stopped honouring ``PATH`` for
  *dependent* DLLs, so ``libgfortran-5.dll`` sitting next to ``gfortran.exe`` --
  on ``PATH``, findable by every other tool -- is invisible to
  ``ctypes.CDLL`` and the load fails with a bare ``FileNotFoundError`` naming
  the *library we just built* rather than the dependency that is missing.  The
  directory has to be registered with :func:`os.add_dll_directory`, and the
  handle that returns has to stay alive for as long as the library is loaded.

Nothing here hard-codes a compiler location.  The runtime directories are
discovered by asking the ``gfortran`` that is actually on ``PATH`` (honouring
``FC``) where its own runtime lives.
"""
