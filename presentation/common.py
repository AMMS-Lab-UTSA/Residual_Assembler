...
PRESENTATION = RA_ROOT / "presentation"
...
def require_worktree_imports() -> Dict[str, str]:
    """Refuse to run against packages imported from some other checkout."""
    ...
    if not origins["umat_oti"].startswith(um):
        raise SystemExit(f"umat_oti imported from {origins['umat_oti']}, not {um}; "
                         "set UMAT_OTI_REPO and PYTHONPATH consistently")
    return origins
